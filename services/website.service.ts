...
/**
 * Website security scanner — mock implementation.
 * Future: Shodan + SSL Labs + Google Safe Browsing + CVE databases
 */
export async function scanWebsite(url: string): Promise<WebsiteScanResult> {
  ...
  const httpsStatus = url.startsWith("https://") || !url.startsWith("http://");
